...
const SchoolsPage = () => {
  ...
  const filteredSchools = schools.filter((school) => {
    const byKeyword =
      search.keyword === "" ||
      school.name.toLowerCase().includes(search.keyword.toLowerCase());
    const byLevel = search.level === "" || school.level === search.level;
    const byStatus = search.status === "" || school.status === search.status;
    const byCapacity =
      search.capacity === "" || school.capacity >= parseInt(search.capacity);
            ...
            <table className="min-w-full table-auto">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-2 text-start">Available slots</th>
                </tr>
              </thead>
              <tbody>
                {filteredSchools.map((school, index) => (
                  <tr key={index} className="border-t border-gray-300">
                    ...
                    <td className="px-4 py-2">
                      {school?.combinations?.join(", ") ?? "-"}
                    </td>
                    <td className="px-4 py-2">{school?.capacity}</td>
                    <td className="px-4 py-2">
                      {typeof school.availableSlots === 'string' ? (
                        school.availableSlots
                      ) : (
                        Object.entries(school.availableSlots).map(([combination, slots]) => (
                          <div key={combination}>
                            {combination} ({slots.remainingSlots} remaining)
                          </div>
                        ))
                      )}
                    </td>
